    ...
    <>
      ...
      <div className="relative z-[2] text-foreground">
        <div className="max-w-[1200px] mx-2 md:mx-4 lg:mx-auto flex justify-center items-center flex-col border-x border-foreground/10">
          <header wb-component="smart-animate" className="w-full flex flex-col items-center gap-5 md:gap-[1.625rem] py-6 md:py-14 border-b border-foreground/10">
            ...
            <div className='flex flex-col gap-5 md:gap-8'>
              {posts.slice(0, visibleCount).map((post, index) => (
                <>
                  ...
                  >
                    <Link to={`/blog/${post.slug}`} className="relative w-full aspect-[179/80] md:aspect-auto md:w-40 rounded border border-foreground/5 sm:self-stretch sm:flex-shrink-0 overflow-hidden bg-muted">
                      <img
                        src={post.heroImage}
                        alt={post.title}
                    ...
                    <div className="flex w-full md:py-[0.625rem] justify-between">
                      <div className="flex flex-col px-3 md:px-0 gap-2 w-full md:w-[80%]">
                        <div className="inline-flex items-center gap-2 text-link font-medium text-text-medium">
                          ...
                          <span className='block lg:hidden' aria-hidden="true">•</span>

                          <span>{post.readingMinutes} min read</span>
                        </div>

                        <Link to={`/blog/${post.slug}`} className="group/title inline-flex md:text-balance">
                  ...
                  { index === posts.length - 1 ? '' : <div className='block md:hidden h-[1px] w-full bg-foreground/10' /> }
                </>
              ))}
