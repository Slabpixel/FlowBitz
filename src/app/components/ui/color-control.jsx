...
const parseToRgb = (colorStr) => {
  ...
  const hex8 = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(colorStr)
  if (hex8) {
    return {
  ...
  return (
    <div className="flex items-center gap-2 py-[0.375rem]">
      ...
      <Popover>
        <PopoverTrigger asChild>
          <button
            type="button"
            disabled={disabled}
            className="w-10 h-10 rounded-lg shrink-0 cursor-pointer border border-foreground/10 disabled:cursor-not-allowed disabled:opacity-50 overflow-hidden"
      ...
      <Select value={format} onValueChange={setFormat}>
        <SelectTrigger className="w-[72px] shrink-0 h-10 px-2 py-3 gap-1 text-link font-medium text-foreground bg-transparent">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
      ...
      {format === 'rgb' ? (
        <div className="flex gap-1.5 flex-1 min-w-0">
          <Input type="number" value={rgb.r} onChange={(e) => handleRgbFieldChange('r', e.target.value)} disabled={disabled} className={INPUT_CLASS} min="0" max="255" />
          <Input type="number" value={rgb.g} onChange={(e) => handleRgbFieldChange('g', e.target.value)} disabled={disabled} className={INPUT_CLASS} min="0" max="255" />
          <Input type="number" value={rgb.b} onChange={(e) => handleRgbFieldChange('b', e.target.value)} disabled={disabled} className={INPUT_CLASS} min="0" max="255" />
        </div>
      ) : format === 'hsl' ? (
        <div className="flex gap-1.5 flex-1 min-w-0">
          <Input type="number" value={hsl.h} onChange={(e) => handleHslFieldChange('h', e.target.value)} disabled={disabled} className={INPUT_CLASS} min="0" max="360" />
          <Input type="number" value={hsl.s} onChange={(e) => handleHslFieldChange('s', e.target.value)} disabled={disabled} className={INPUT_CLASS} min="0" max="100" />
          <Input type="number" value={hsl.l} onChange={(e) => handleHslFieldChange('l', e.target.value)} disabled={disabled} className={INPUT_CLASS} min="0" max="100" />
        </div>
      ) : (
        <Input
          type="text"
          ...
          disabled={disabled}
          className="flex-1 h-10 text-link text-foreground font-mono"
          placeholder="#000000"
        />
      )}
...
const ColorControl = ({ value, onChange, disabled, supportsAlpha = false, multiple = false }) => {
  if (multiple) {
    return <MultiColorControl value={value} onChange={onChange} disabled={disabled} />
  }
